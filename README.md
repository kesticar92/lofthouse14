...
Abre [http://localhost:43127](http://localhost:43127) (dev). El panel admin está en `/admin`.

## Scripts

| Comando                | Descripción               |
| ---------------------- | ------------------------- |
| `npm run dev`          | Servidor de desarrollo (:43127) |
...
- Multi-tenant: membership en `org_members`; migraciones `017`–`028` en [`supabase/migrations/`](supabase/migrations/). Sin migrar, catálogo/booking/admin caen a **seed/local mock** (evaluable).
...
2. En **SQL Editor**, ejecuta las migraciones en orden bajo [`supabase/migrations/`](supabase/migrations/) (empezando por [`001_profiles_audit.sql`](supabase/migrations/001_profiles_audit.sql); multi-tenant `017`–`020`; plataforma `021`–`028`).
