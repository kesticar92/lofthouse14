...
import { type AutomationEventType } from "@/lib/crm/templates";
import { LOFTHOUSE_ORGANIZATION_ID } from "@/lib/tenant/constants";
import {
  listAutomationRuns,
  runAutomation,
} from "@/lib/crm/automation-runner";
...
export async function GET() {
  ...
  if (organizationId) {
    ...
    if (data && data.length > 0) {
      return Response.json({ mode: "supabase", events: data });
    }
  }

  const local = listAutomationRuns(50);
  return Response.json({
    mode: "local",
    events: local,
    note: "Runner local (log + notification stub) — sin WhatsApp real",
  });
}
...
export async function POST(req: Request) {
  ...
  const event = runAutomation({
    eventType: body.event_type ?? "custom",
