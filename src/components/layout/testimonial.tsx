...
const StarIcon = ({ fill = "currentColor" }: { fill?: string }) => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill={fill} xmlns="http://www.w3.org/2000/svg">
...
const GoogleLogoPlaceholder = () => (
  <div className="flex items-center gap-1.5" title="Google">
    ...
    <GoogleLogo />
    <span className="text-[10px] font-bold tracking-tight text-zinc-600 dark:text-zinc-400">Google</span>
  </div>
);
...
const BookingLogoPlaceholder = () => (
  <div className="flex items-center gap-1.5" title="Booking.com">
    ...
    <span className="text-[10px] font-bold tracking-tight text-[#003580] dark:text-blue-400">Booking</span>
  </div>
);
              ...
              {props.testimonials.map((testimonial, i) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    ...
                    {(testimonial.source === "google" || testimonial.source === "booking") && (
                      <div className="flex flex-col items-end gap-1.5">
                        <div className="flex gap-0.5">
                          {[...Array(5)].map((_, starIdx) => (
                            <StarIcon 
                              key={starIdx} 
                              fill={starIdx < Math.round(testimonial.rating || 5) ? "#FBBC05" : "#E5E7EB"} 
                            />
                          ))}
                        </div>
